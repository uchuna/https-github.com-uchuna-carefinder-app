...
// Reactive variables for email, password, and error handling
const email = ref('');
const password = ref('');
...
// Function to add hospitals to Firebase Firestore
export const addHospitalsToFirebase = async () => {
  try {
...
// Define reactive variables for selected state and provider
export const selectedState = ref('');
export const selectedProvider = ref('');

// Function to filter hospitals by state
const filterHospitalsByState = (hospitals: any[], state: string) => {
  return hospitals.filter(hospital => hospital.state.includes(state));
};

// Function to filter hospitals by provider
const filterHospitalsByProvider = (hospitals: any[], provider: string) => {
  return hospitals.filter(hospital => hospital.provider === provider);
};

// Computed property to get filtered hospitals based on selected state
export const filteredHospitalsByState = computed(() => {
  if (!selectedState.value) return hospitals; // Return all hospitals if no state is selected
  return filterHospitalsByState(hospitals, selectedState.value);
});

// Computed property to get filtered hospitals based on selected provider
export const filteredHospitalsByProvider = computed(() => {
  if (!selectedProvider.value) return hospitals; // Return all hospitals if no provider is selected
  return filterHospitalsByProvider(hospitals, selectedProvider.value);
});

export { app, db, email, password, error };